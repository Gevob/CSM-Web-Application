...
const EditPage = (props) => {
  ...
  useEffect(() => {
    API.getPage(pageId)
      .then((page) => {
        setTitle(page.title);
        setDate(page.date);
        setInputFields(page.blocks);
        const tmp = Array.from({ length: page.blocks.length }, () => "");
  ...
  const renderAuthors = (authors) => {
    return authors.map((author, index) => {
      return (
        <option key={index} value={author.nome}>
          {author.nome}
        </option>
      );
    });
  ...
  const insertPage = async (event) => {
    ...
    if (inputCheck.every((b) => b === "")) {
      let final_author;
      if (author !== user.name) final_author = author;
      else final_author = user.name;
      const page = { title, date, inputFields, author: final_author };
      API.editPage(pageId, page)
    ...
    >
      ...
      <Form noValidate onSubmit={insertPage}>
        <div className="d-flex justify-content-center">
          ...
          {user.type === 1 ? (
            <Form.Group>
              <Form.Label className="d-flex flex-row">Autore:</Form.Label>
              <Form.Select
                value={author}
        ...
        <Container className="d-flex justify-content-center align-items-center flex-column">
          {inputFields.map((input, index) => {
            return (
              <div
                className="d-flex justify-content-center setdrag mb-3"
                draggable
        ...
        <Button
          className="my-2  custom-back-button button-size"
          onClick={() => navigate("/")}
        >
          Home
        </Button>
